...
- [frontend-cache.md](./frontend-cache.md): Production frontend cache-control and deploy-runtime notes covering Nginx ownership of static pages, unknown-route redirect ownership, live activation through Nginx reloads, the env-backed finance server-block template, deploy-time asset versioning, backend health verification, the local SSH helpers, and the exact API routes that still proxy to Express.
