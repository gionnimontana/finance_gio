...
- [zero-knowledge-auth-plan.md](./zero-knowledge-auth-plan.md): Step-by-step design for the zero-knowledge identity refactoring, full local secure state aggregation, and a secure client-mediated migration pathway for legacy accounts.
...
- [scraper-runtime.md](./scraper-runtime.md): Fallback-provider runtime notes covering fetch-only API providers, KID-derived ETF risk scraping, computed crypto and gold risk scoring from Yahoo Finance history, lazy page startup, browser-profile hardening for automation-sensitive sites, cache TTLs, stale-value reuse, best-effort shared cache persistence, and deterministic scraper regression tests.
