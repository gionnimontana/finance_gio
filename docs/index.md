...
- [data-model.md](./data-model.md): Cross-cutting notes about password-derived user storage, the shared `isinRiskCache.json`, `cryptoRiskCache.json`, and `goldRiskCache.json` files, account deletion, view-group ordering, schema-cache invalidation, browser-local caches, and cross-device synchronization behavior.
...
- [scraper-runtime.md](./scraper-runtime.md): Fallback-provider runtime notes covering fetch-only API providers, KID-derived ETF risk scraping, computed crypto and gold risk scoring from Yahoo Finance history, lazy page startup, browser-profile hardening for automation-sensitive sites, cache TTLs, stale-value reuse, and deterministic scraper regression tests.
