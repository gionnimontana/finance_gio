...
- [frontend-cache.md](./frontend-cache.md): Production frontend cache-control and deploy-runtime notes covering Nginx ownership of static pages, the checked-in finance server-block extract, deploy-time asset versioning, backend health verification, the local SSH helper, and the exact API routes that still proxy to Express.
