...
- [data-model.md](./data-model.md): Cross-cutting notes about password-derived user storage, the shared `isinRiskCache.json`, `cryptoRiskCache.json`, and `goldRiskCache.json` files, `assetsSchema.riskOverrides` for `Other` assets, account deletion, view-group ordering, schema-cache invalidation, failed-refresh cache preservation, last-successful refresh baselines, browser-local caches, and cross-device synchronization behavior.
