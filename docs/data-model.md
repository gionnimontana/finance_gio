...
- When a live dashboard refresh returns failures and omits same-schema dynamic asset rows, the frontend preserves the last cached rows for those missing assets before it overwrites `localStorage`, so the dashboard stays readable while still surfacing the refresh error banner.
