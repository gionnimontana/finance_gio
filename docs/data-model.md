...
- The dashboard keeps a browser-local `portfolio` snapshot in `localStorage` for fast reloads and a separate `portfolioLastSuccessfulSnapshot` plus `portfolioLastUpdate` baseline for refresh-diff comparisons. Before reusing the visible snapshot, the frontend compares the cached `schemaCacheKey` against the backend schema and refreshes when the asset or group shape changed.
...
- The same frontend recovery path also treats same-schema asset ids that disappear from a refresh payload as an implicit partial failure even if the backend forgot to populate `failures`, so a silent partial payload cannot overwrite the visible cache or the last-successful refresh baseline as a clean success.
- That same partial-refresh path now leaves `portfolioLastSuccessfulSnapshot` and `portfolioLastUpdate` untouched, so the next clean refresh still computes grouped and total diffs against the last fully successful refresh instead of a mixed partial cache.
