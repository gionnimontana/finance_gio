...
- A zero-knowledge migration foundation now also exists in parallel: the backend can store one opaque `userBlob.json` ciphertext envelope per client-derived `X-User-Id`, while explicit `/market/quotes` and `/market/risk-indicators` routes accept only the asset descriptors needed for stateless market lookups.
...
- The new opaque blob and stateless market-data routes are not the primary UI flow yet. The checked-in frontend now includes shared browser helpers for deriving zero-knowledge user ids, encrypting opaque state envelopes, and calling those new routes, but the current pages still render from the legacy password-authenticated schema, history, and portfolio endpoints.
