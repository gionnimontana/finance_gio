...
- Crypto and gold risk scoring currently use 365 days of Yahoo Finance daily closes, score annualized realized volatility over the last 90 days, score max drawdown over the last 365 days, and keep the higher bucket on a fixed 1-7 scale so the result stays deterministic and fetch-first. Gold history currently comes from Yahoo Finance gold futures (`GC=F`) while the live EUR-per-gram quote still comes from goldpreis.de.
