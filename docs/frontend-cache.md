...
- The deployed Nginx config is expected to proxy only exact backend endpoints to Express: `/health`, `/auth/generate`, `/auth/validate`, `/auth/user`, `/user/blob`, `/market/quotes`, `/market/risk-indicators`, `/portfolio`, `/portfolio/history`, `/portfolio/stream`, `/assets/schema`, `/assets/isin-risk`, `/assets/risk-indicators`, `/assets/view-groups`, and `/assets/risk-overrides`.
...
- The newly added zero-knowledge foundation routes `/user/blob`, `/market/quotes`, and `/market/risk-indicators` also require exact proxy entries in the production template, even before the main UI switches away from the legacy password-based portfolio flow.
- Because HTML shells always revalidate and client-side auth decisions still happen after the shell arrives, the shared loading overlay is part of the source frontend rather than the deploy rewrite step, ensuring local dev, tests, and production all mask the same cold-boot redirect path.
