...
- The current finance site `server { ... }` block extracted from production lives in `../finance.gingergio.it.nginx` so future Nginx include refactors can start from the deployed behavior instead of a hand-rebuilt copy.
...
- `finance.gingergio.it.nginx` is a checked-in extract of the live production server block, but `/etc/nginx/nginx.conf` on the host remains the source of truth until the remote config is updated to include that file explicitly.
...
- [../deploy.sh](../deploy.sh)
- [../finance.gingergio.it.nginx](../finance.gingergio.it.nginx)
