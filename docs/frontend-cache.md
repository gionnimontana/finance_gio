...
- `npm run deploy:remote` intentionally shells into `PFB_DEPLOY_APP_PATH` and delegates to the checked-in `deploy.sh`, so the remote host keeps using the same deploy workflow, Nginx template rendering, and post-restart health verification as a manual SSH session.
