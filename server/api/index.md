...
- [index.js](./index.js): Reads, validates, migrates, and writes `assetsSchema.json` and `historicalData.json` for a user, including per-user `riskOverrides` values used only by `Other` assets, derives a schema cache key used by the dashboard to invalidate stale local portfolio snapshots, and skips current-month history overwrites when a live refresh still reports scraper failures.
