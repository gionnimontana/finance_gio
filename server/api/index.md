...
- [index.js](./index.js): Reads, validates, migrates, and writes `assetsSchema.json` and `historicalData.json` for a user, including per-user `riskOverrides` values used only by `Other` assets, and derives a schema cache key used by the dashboard to invalidate stale local portfolio snapshots.
...
- [userBlob.js](./userBlob.js): Persists validated opaque ciphertext envelopes under the per-user data root using a client-derived user id, without parsing decrypted user content on the backend.
