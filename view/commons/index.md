...
- [utils.js](./utils.js): Common auth, fetch, centralized currency-aware whole-number full-value or compact absolute-value formatting with persisted Settings preferences plus opt-in forced-compact labels and no-currency overrides for exception views, privacy-aware percentage helpers, shared saved-or-default view-group color resolution, shared authenticated assets-schema, history, and asset-risk fetch helpers, shared ATH mood-title helpers, shared footer and banner utilities, and the helper that dismisses the loading overlay only after the first page render is ready.
