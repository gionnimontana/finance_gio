...
- [script.js](./script.js): Portfolio fetching, first-load asset-level streaming feedback, refresh streaming, schema-aware cache reconciliation, and DOM rendering logic, including precise crypto subrow rendering, shared absolute-value formatting that defaults to full values and can switch to compact `k` and `m` labels from Settings, the ATH distance summary, the runtime dashboard title mood icon, and chart-color-tinted group cards.
